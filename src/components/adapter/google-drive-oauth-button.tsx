...
export function GoogleDriveOAuthButton({ credentialId, authorized, onAuthorized }: GoogleDriveOAuthButtonProps) {
    ...
    useEffect(() => {
        ...
        let active = true;
        fetch("/api/adapters/google-drive/validate-token", {
            method: "POST",
    ...
    if (authorized && tokenState === "checking") {
        return (
            <Alert className="border-muted items-center [&>svg]:translate-y-0">
                ...
                <AlertDescription className="text-muted-foreground">Verifying Google Drive authorization...</AlertDescription>
            </Alert>
        );
    }
        ...
        return (
            <Alert className="border-amber-500/30 bg-amber-500/5 items-center [&>svg]:translate-y-0">
                ...
                <AlertDescription className="flex items-center justify-between">
                    <span className="text-amber-600">Authorization expired. Please re-authorize with Google.</span>
                    <Button
                        type="button"
                        variant="outline"
                        size="sm"
